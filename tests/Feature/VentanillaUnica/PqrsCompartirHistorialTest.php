...
class PqrsCompartirHistorialTest extends TestCase
{
    use RefreshDatabase;
    ...
    protected function setUp(): void
    {
        ...
        $this->pqrs = VentanillaPqrs::create([
            ...
            'asunto' => 'PQRS de prueba para compartir',
    ...
    /** @test */
    public function puede_compartir_pqrs_con_otro_usuario()
    {
        $response = $this->actingAs($this->user)
            ->postJson("/api/ventanilla/pqrs/{$this->pqrs->id}/compartir", [
        ...
        $this->assertDatabaseCount('ventanilla_pqrs_compartir_historial', 1);
        $this->assertDatabaseHas('ventanilla_pqrs_compartir_historial', [
    ...
    /** @test */
    public function falla_si_falta_usuario_destino()
    {
        $response = $this->actingAs($this->user)
            ->postJson("/api/ventanilla/pqrs/{$this->pqrs->id}/compartir", [
                'users_cargos_destino_id' => $this->userCargo2->id,
        ...
        $response->assertStatus(422)
            ->assertJsonValidationErrors(['usuario_destino_id']);
    }

    /** @test */
    public function puede_listar_historial_compartir_de_un_pqrs()
    {
        VentanillaPqrsCompartirHistorial::create([
            'pqrs_id' => $this->pqrs->id,
            'usuario_origen_id' => $this->user->id,
            'users_cargos_destino_id' => $this->userCargo2->id,
            'usuario_destino_id' => $this->user2->id,
        ...
        $response = $this->actingAs($this->user)
            ->getJson("/api/ventanilla/pqrs/{$this->pqrs->id}/compartir");
    ...
    {
        $response = $this->getJson("/api/ventanilla/pqrs/{$this->pqrs->id}/compartir");
        $response->assertStatus(401);

        $response = $this->postJson("/api/ventanilla/pqrs/{$this->pqrs->id}/compartir", []);
        $response->assertStatus(401);
    }
    ...
    {
        $response = $this->actingAs($this->user)
            ->postJson("/api/ventanilla/pqrs/99999/compartir", [
                'users_cargos_destino_id' => $this->userCargo2->id,
                'usuario_destino_id' => $this->user2->id,
            ]);
