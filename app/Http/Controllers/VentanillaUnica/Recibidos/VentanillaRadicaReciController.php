...
class VentanillaRadicaReciController extends Controller
{
    ...
    public function __construct(
        ...
        protected RadicadoConsecutivoService $consecutivoService,
    ) {
     ...
     * }
     */
    public function store(StoreRadicadoReciboRequest $request)
    {
        try {
            DB::beginTransaction();
            ...
            $num_radicado = $this->consecutivoService->reservarRecibido();
     ...
     * }
     */
    public function update($id, StoreRadicadoReciboRequest $request)
    {
        try {
            DB::beginTransaction();
    ...
    {
        try {
            $cacheKey = 'ventanilla_recibidos_estadisticas';
    ...
    /**
     * Busca radicados por contenido OCR (búsqueda full-text)
     *
     ...
     * @bodyParam per_page Resultados por página (default: 20)
     *
     * @response 200 {
     *   "status": true,
     *   "data": {
     ...
     *       {
     *         "id": 1,
     *         "num_radicado": "2026-000001",
     *         "asunto": "Solicitud de información",
     *         "ocr_resumen": "...texto encontrado...",
     *         "tercero": { ... }
     *       }
     *     ],
     *     "current_page": 1,
    ...
    public function searchByOcr(Request $request): JsonResponse
    {
        ...
        $searchOcr = $request->get('searchOcr', '');

        // Usar searchOcr si está presente, si no usar q
        $query = $searchOcr ?: $searchTerm;
        ...
        // Si es búsqueda específica de OCR
        if ($searchOcr) {
            $radicados = VentanillaRadicaReci::where('ocr', 'LIKE', "%{$query}%")
                ->limit(20)
                ->get();
        } else {
            // Búsqueda general
            $radicados = VentanillaRadicaReci::where('num_radicado', 'LIKE', "%{$query}%")
    ...
    /**
     * Prepara el query para búsqueda fulltext en modo BOOLEAN
     */
    private function prepareFullTextQuery(string $query): string
    {
        // Dividir en palabras y agregar wildcard a cada una
    ...
    /**
     * Extrae un fragmento del texto OCR donde se encuentra la búsqueda
     */
    private function extractOcrContext(?string $ocr, string $query, int $contextLength = 200): ?string
    {
        ...
        $end = min(mb_strlen($ocr), $position + mb_strlen($query) + ($contextLength / 2));

        // Ajustar para no cortar palabras
        if ($start > 0) {
            $spacePos = mb_strpos($ocr, ' ', $start);
            if ($spacePos !== false && $spacePos < $position) {
                $start = $spacePos + 1;
            }
        }

        if ($end < mb_strlen($ocr)) {
            $spacePos = mb_strrpos(mb_substr($ocr, 0, $end), ' ');
            if ($spacePos !== false && $spacePos > $start) {
                $end = $spacePos;
            }
        }

        $fragment = mb_substr($ocr, $start, $end - $start);

        // Agregar "..." si hay más texto antes o después
        $prefix = $start > 0 ? '...' : '';
        ...
        try {
            ...
            $eventos = [
                [
                    ...
                    'tipo' => 'radicado_creado',
                    'estado' => $radicado->estado_trabajo,
                    'descripcion' => 'Radicado creado',
                ],
            ];

            if ($radicado->updated_at && $radicado->updated_at->gt($radicado->created_at)) {
                $eventos[] = [
